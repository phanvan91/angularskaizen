...
export class IssueComponent implements OnInit, OnDestroy {
    ...
    initSelect2() {
        this.soHieuChungTuService.getByType(this.loai_ct,
            this.loai_kho).takeWhile(() => this.alive).subscribe((res: any ) => {
            if (res) {
                this.data_ct = res;
            }
        });
        ...
        if (!this.chungTuId) {
            ...
            if (this.deNghiCapVatTuId) {
                ...
                this.khoService.khoSelected.takeWhile(() => this.alive).subscribe((res: any ) => {
                    if (res.length) {
                        this.issue.kho_xuat = res.find( x => x.loai_kho === this.loai_kho);
    ...
    getDanhSachLinkien(keyWord) {
        if (this.lkKeyWord !== keyWord) {
            ...
            this.linhKienService.search(keyWord, this.issue.kho_xuat.id,
                this.isKhoTot).takeWhile(() => this.alive).subscribe((res: any) => {
                this.data_lk = [];
                this.data_lk_map = new Object();
                if (res.length > 0) {
                    for (let i = 0; i < res.length; i++) {
    ...
    changeDonDatHang(value) {
        this.issue.don_dat_hang = value;
        this.issue.hoa_don_dat_hang_id = value.value;
        this.issue.linh_kiens = [];
        this.donDatHangService.getChiTiet(this.issue.id).takeWhile(() => this.alive).subscribe((res: any) => {
            if (res && res.length > 0) {
                let lk;
                this.issue.linh_kiens = [];
                this.issue.tong_so_tien_truoc_thue = 0;
                for (let i = 0; i < res.length; i++) {
                    ...
                    lk.so_luong_nhap = lk.so_luong;
                    this.issue.tong_so_tien_truoc_thue += lk.so_luong_nhap * lk.don_gia;
                    this.issue.linh_kiens[i] = lk;
                }
            }
        });
    }
    ...
    deleteReceipt() {
        this.chungTuService.delete({'chung_tu_id' : this.issue.id,
            ...
            'phieu_sua_chua_id': this.issue.phieu_sua_chua_id})
    ...
    getDocumentData() {
        ...
        data.loai_ct = environment.loai_chung_tu.phieu_xuat_kho;
        ...
        for (let i = 0; i < this.issue.linh_kiens.length; i++) {
            ...
            data.linh_kiens.push({'id': lk.id, 'linh_kien_id': lk.linh_kien_id, 'so_luong_yc': lk.so_luong_yc, 'so_luong_thuc': lk.so_luong_thuc,
                'loai_giao_dich': environment.loai_giao_dich.xuat,
                'don_gia' : lk.don_gia,
                'loai_ct': environment.loai_chung_tu.phieu_xuat_kho,
                'ghi_chu': lk.ghi_chu,
