...
@Component({
  selector: 'app-ds-phieu-nhap-kho-thanh-pham',
  templateUrl: './ds-phieu-nhap-kho-thanh-pham.component.html',
  styleUrls: ['./ds-phieu-nhap-kho-thanh-pham.component.scss']
})
export class DsPhieuNhapKhoThanhPhamComponent implements OnInit {
  ...
  ngOnInit() {
    this.khoService.khoSelected.subscribe((res:any)=>{
      if(res.length){
        this.khoNhap = res.find(x=>x.loai_kho == environment.loai_kho.kho_thanh_pham );
        this.onLoadAllData();
      }
    })
  }
  ...
  onSearch(event){
    ...
    this.chungTuService.filterListChungTuKhoTot(environment.loai_chung_tu.phieu_nhap_kho,this.khoNhap.id,
      this.searchForm.ngay_bat_dau, this.searchForm.ngay_ket_thuc).subscribe((res: any)=>{
        if(res){
