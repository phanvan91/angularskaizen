...
@Component({
  selector: 'app-ds-phieu-xuat-kho-thanh-pham',
  templateUrl: './ds-phieu-xuat-kho-thanh-pham.component.html',
  styleUrls: ['./ds-phieu-xuat-kho-thanh-pham.component.scss']
})
export class DsPhieuXuatKhoThanhPhamComponent implements OnInit {
  khoXuat;
  chungTuList;
  chungTuPagination;
  ...
  ngOnInit() {
    this.khoService.khoSelected.subscribe((res:any)=>{
      if(res.length){
        this.khoXuat = res.find(x=>x.loai_kho ==  environment.loai_kho.kho_thanh_pham);
        this.onLoadAllData();
      }
    })
  }
