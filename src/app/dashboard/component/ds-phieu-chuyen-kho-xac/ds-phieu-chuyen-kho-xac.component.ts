...
@Component({
  selector: 'app-ds-phieu-chuyen-kho-xac',
  templateUrl: './ds-phieu-chuyen-kho-xac.component.html',
  styleUrls: ['./ds-phieu-chuyen-kho-xac.component.scss']
})
export class DsPhieuChuyenKhoXacComponent implements OnInit {
  ...
  ngOnInit() {
    this.khoService.khoSelected.subscribe((res:any)=>{
      if(res.length){
        this.kho = res.find(x=>x.loai_kho == environment.loai_kho.kho_xac);
        this.onLoadAllData();
      }
    })
  }
