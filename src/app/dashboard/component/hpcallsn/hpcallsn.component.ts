...
@Component({
  selector: 'app-hpcallsn',
  templateUrl: './hpcallsn.component.html',
  styleUrls: ['./hpcallsn.component.scss']
})
export class HpcallsnComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    hpCallSubmit(event) {
        ...
        this.hpcallService.actionHpcall(data).takeWhile(() => this.alive).subscribe((res: any) => {
            console.log(res);
            this.HpCall.splice(this.ActionIndex, 1);

            (<any>$('#edit_hpcallsn')).modal('hide');
        }, err => console.log(err));
