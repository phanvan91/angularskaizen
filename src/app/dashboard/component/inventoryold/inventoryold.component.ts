...
@Component({
  selector: 'app-inventoryold',
  templateUrl: './inventoryold.component.html',
  styleUrls: ['./inventoryold.component.scss']
})
export class InventoryoldComponent implements OnInit {
    ...
    ngOnInit() {
      ...
      this.editform = new FormGroup({
        ...
        this.khoService.khoSelected.subscribe((resKho: any ) => {
            if (resKho.length) {
                this.khoNhap = resKho.find( x => x.loai_kho === environment.loai_kho.kho_xac );
                console.log(this.khoNhap);
                if (this.khoNhap.id > 0) {
                    this.khoService.getTonKhoXau(this.page, this.khoNhap.id, this.linh_kien_id )
    ...
    show_page(index) {
        this.page = index;
        this.khoService.getTonKhoXau(this.page, this.khoNhap.id, this.linh_kien_id).takeWhile(() => this.alive).subscribe((res: any) => {
            this.TonKho = res.data;
            this.total = res.total;
            this.pages = res.last_page;
            this.min = Math.max(this.page - 4, 1);
